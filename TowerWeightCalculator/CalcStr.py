...
tower_data = {
    "NS5": {
        "BTB_options": {
            ...
            "BASIC TOWER BODY WHEN WITH -5M LEG EXTENSION": 7658.179,
        },
        "BE_values": {
            "E0": 0.0,
            "E3": 1398.814,
    ...
    "LA5": {
        "BTB_options": {
            "BASIC TOWER BODY WHEN WITH -4M & -5M LEG EXTENSIONS": 11175.495,
            "BASIC TOWER BODY WHEN WITH BE OR -3M,-2M,-1M,+0M,+1M AND +2M LEG EXTENSIONS": 11173.535,
        },
        "BE_values": {
            "E0": 0.0,
            "E3": {
                "with -4M & -5M leg extensions": 2174.258,
                "with -3M to +2M leg extensions": 2151.228
            },
            "E6": {
                "with -4M & -5M leg extensions": 3596.067,
                "with -3M to +2M leg extensions": 3571.704
            },
            "E9": {
                "with -4M & -5M leg extensions": 4696.969,
                "with -3M to +2M leg extensions": 4693.660
            }
        },
        "Leg_values": {
            "-5M": 187.406,
...
with st.sidebar.expander("Tower Selection", expanded=True):
    ...
    selected_be = st.selectbox("Body Extension", be_options, key='body_extension')

    # Leg Extension selection for each leg
    leg_extensions = []
    for i in range(1, 5):
...
def calculate_tower_weight():
    ...
    if tower_type == "NS5":
        ...
        elif any(ext == "-5M" for ext in leg_extensions):
        ...
        if tower_type == "HA5/DE5":
            ...
            if "-4M" in leg_extensions or "-5M" in leg_extensions:
                ...
                BTB_weight = data["BTB_options"]["BASIC TOWER BODY WHEN WITH BE OR -3M,-2M,-1M,+0M,+1M AND +2M LEG EXTENSIONS"]
...
if st.session_state['calculation_results']:
    ...
    # Styling: Total Weight in red and bold, BB+BE and ALL Legs in blue
    df_styled = df.style.applymap(lambda x: 'font-weight: bold; color: red;' if x == df['Total Weight'].max() else '',
                                  subset=['Total Weight'])\
                         .applymap(lambda x: 'color: blue;' if x == df['BB+BE Weight'].max() else '',
                                  subset=['BB+BE Weight'])\
                         .applymap(lambda x: 'color: blue;' if x == df['ALL Legs'].max() else '',
                                  subset=['ALL Legs'])

    # Centering all columns except SNO and Tower Type
    df_styled = df_styled.set_properties(subset=df.columns.difference(['SNO', 'Tower Type']), **{'text-align': 'center'})
